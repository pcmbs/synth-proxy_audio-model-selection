...
PATH_TO_PLUGIN = DATA_PATH / "TAL-NoiseMaker.vst3"
PATH_TO_PRESETS = DATA_PATH / "TAL-NoiseMaker vst3"
...
if __name__ == "__main__":
    ...
    for preset in paths_to_presets:
        ...
        for param in synth._parameters[:89]:  # pylint: disable=W0212
            presets[preset_name].append({"index": param.index, "name": param.name, "value": param.raw_value})

    if EXPORT_PRESETS:
        with open(DATA_PATH / "tal_noisemaker_presets.json", "w", encoding="utf-8") as f:
            json.dump(presets, f)

    print(f"noisemaker_presets.json saved in {str(DATA_PATH)}")
